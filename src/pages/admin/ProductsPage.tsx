...
export default function ProductsPage() {
  ...
  // Upload photos (like CarForm)
  const uploadProductPhotos = async (files: File[], productId: string, category: string): Promise<string[] | null> => {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!productId || !category) {
      alert('Заполните название и категорию для загрузки фото');
      e.target.value = '';
      return;
    }
    ...
    try {
      const uploadedPaths = await uploadProductPhotos(files, productId, category);
        ...
        <div className="max-w-7xl mx-auto space-y-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold tracking-tight">LOFT FIRE <span className="text-orange-600">PRO</span></h1>
            </div>
